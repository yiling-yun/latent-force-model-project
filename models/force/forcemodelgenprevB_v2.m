function [estpara] = forcemodelgenprevB_v2(estpara,dataraw)
% Pass 2 estimation for Agent B (v2).
% Identical to forcemodelgenprevB (v1) except uses LJfuncParaEstself3_v2
...
% Animation-level budget (Agent B mean speed, computed once)
sp_b = sqrt(diff(data(:,3)).^2 + diff(data(:,4)).^2);
if mean(sp_b) < 0.10; budget_a = 1000; else; budget_a = 3000; end
...
for fi = 1+intv:intv:framenum-intv
    ...
    indx1 = indx1(1);   % refine ONE tied-best seed; all ties share the same
                        % objective. Avoids wasted fminsearch runs when a
                        % degenerate (e.g. near-static swapped) agent makes the
                        % grid landscape flat with many ties.
    ...
    devvalinit = LJfuncParaEstself3_v2(initialParams,x);       % <-- v2 objective
    objectiveFunc = @(parameters) LJfuncParaEstself3_v2(parameters,x);  % <-- v2 objective
    ...
    if devval<devvalinit
        ...
        % FIX: fall back to V2 Pass-1 estimate (estparainit), not the
        % loop's last `initialParams` (just whatever seed was tried
        % last). Pairing the latter with devvalinit corrupts the
        % per-window record.
